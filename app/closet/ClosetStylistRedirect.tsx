...
export default function ClosetStylistRedirect(){
 useEffect(()=>{
  ...
  function shouldIntercept(){
   ...
   return p==='/closet'||p==='/closet/wardrobe'||p==='/closet/looks';
  }
  function go(anchor?:string){
   const q=anchor?`?anchor=${encodeURIComponent(anchor)}`:'';
   location.href=`/closet/stylist/${q}`;
  }
  function onClick(e:MouseEvent){
   ...
   go(anchor?decodeURIComponent(anchor):undefined);
  }

  // Important: do not observe DOM/text changes here. The previous MutationObserver
  // redirected the user whenever legacy occasion copy appeared anywhere on the
  // Closet home, which caused spontaneous navigation to Stylist without a click.
