...
export default function LookPage(){
 ...
 return <main className={styles.page}>
  ...
  </section>:<section className={styles.tryOnScreen}><div className={styles.tryHeader}><button onClick={()=>setTryOn(false)}>‹</button><div><span>PROVADOR</span><strong>Veja vestido</strong></div></div><TryOnStage pieces={look} models={avatarModels} avatarId={avatarId} image={tryOnImage} busy={tryOnBusy} error={tryOnError} onAvatarChange={id=>{setAvatarId(id);setTryOnImage('');setTryOnError('')}} onGenerate={generateTryOn} onSwap={p=>{setTryOn(false);setSwap(p)}} onRemix={()=>{setTryOn(false);remix()}} onResetModel={()=>{setTryOnImage('');setTryOnError('')}}/></section>}
