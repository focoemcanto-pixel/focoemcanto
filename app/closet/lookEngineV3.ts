...
function remember(items:StylistPiece[],occasion:string,anchor?:StylistPiece){if(typeof window==='undefined'||!items.length)return;const key=historyKey(occasion,anchor),sig=signature(items),old=recentSignatures(occasion,anchor).filter(x=>x!==sig);sessionStorage.setItem(key,JSON.stringify([sig,...old].slice(0,10)))}
function choose(candidates:StylistPiece[][],occasion:string,profile:StyleProfile,seed:number,anchor?:StylistPiece,avoid:StylistPiece[]=[]){const unique=new Map<string,StylistPiece[]>();for(const c of candidates)if(c?.length)unique.set(signature(c),c);const recent=recentSignatures(occasion,anchor);const ranked=[...unique.values()].map(items=>{const sig=signature(items);let s=score(items,occasion,profile,anchor);if(recent.includes(sig))s-=140-Math.min(80,recent.indexOf(sig)*16);if(avoid.length){const shared=overlap(items,avoid);s-=shared>=.99?180:shared>=.66?58:shared>=.34?20:0}return{items,s}}).sort((a,b)=>b.s-a.s);if(!ranked.length)return[];const completeRanked=ranked.filter(x=>complete(x.items));const pool=completeRanked.length?completeRanked:ranked;const best=pool[0].s;const viable=pool.filter(x=>x.s>=best-22).slice(0,10);const picked=viable[Math.abs(seed)%viable.length]?.items||pool[0].items;remember(picked,occasion,anchor);return picked}
function variants(all:StylistPiece[],occasion:string,profile:StyleProfile,seed:number,anchor?:StylistPiece){const usable=usableWardrobe(all,occasion);const anchorAllowed=anchor&&allowedForOccasion(anchor,occasion)?anchor:undefined;const out:StylistPiece[][]=[];for(let i=0;i<24;i++){const s=seed+i*5;out.push(anchorAllowed?buildStyledLookWithAnchorV2(usable,anchorAllowed,occasion,profile,s) as StylistPiece[]:buildStyledLookV2(usable,occasion,profile,s) as StylistPiece[])}return out}
