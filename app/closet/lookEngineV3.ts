...
function choose(candidates:StylistPiece[][],occasion:string,profile:StyleProfile,seed:number,anchor?:StylistPiece){const unique=new Map<string,StylistPiece[]>();for(const c of candidates)if(c?.length)unique.set(signature(c),c);const ranked=[...unique.values()].map(items=>({items,s:score(items,occasion,profile,anchor)})).sort((a,b)=>b.s-a.s);if(!ranked.length)return[];const best=ranked[0].s,viable=ranked.filter(x=>x.s>=best-7).slice(0,5);return viable[Math.abs(seed)%viable.length].items}

export function buildStyledLookV3(all:StylistPiece[],occasion:string,profile:StyleProfile={},seed=0){
 const variants:StylistPiece[][]=[];for(let i=0;i<7;i++)variants.push(buildStyledLookV2(all,occasion,profile,seed+i) as StylistPiece[]);return choose(variants,occasion,profile,seed);
}
export function buildStyledLookWithAnchorV3(all:StylistPiece[],anchor:StylistPiece,occasion:string,profile:StyleProfile={},seed=0){
 const variants:StylistPiece[][]=[];for(let i=0;i<7;i++)variants.push(buildStyledLookWithAnchorV2(all,anchor,occasion,profile,seed+i) as StylistPiece[]);return choose(variants,occasion,profile,seed,anchor);
}
