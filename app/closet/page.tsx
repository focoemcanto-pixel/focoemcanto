...
type Sheet = 'add' | 'edit' | 'occasion' | 'wardrobe' | 'closetStatus' | null;
...
export default function ClosetPage(){
 ...
 {sheet==='closetStatus'&&<><div className={styles.sheetHeader}><div><span className={styles.kicker}>Primeiro look</span><h3>{pieces.length?'Seu closet está começando.':'Seu stylist está pronto.'}</h3></div><button onClick={()=>setSheet(null)}>×</button></div><p className={styles.sheetIntro}>{pieces.length?'Faltam só algumas bases para eu montar uma combinação que faça sentido.':'Só faltam suas roupas. Você pode começar com três peças e testar o app sem cadastrar o guarda-roupa inteiro.'}</p><div className={styles.progressBox}><div className={styles.progressTop}><strong>{readiness} de 3</strong><span>para gerar o primeiro look</span></div><div className={styles.progressBar}><i style={{width:`${(readiness/3)*100}%`}}/></div><div className={styles.requirementList}><div className={hasTop?styles.done:''}><span>{hasTop?'✓':'○'}</span><p><strong>Parte de cima</strong><small>Blusa, camisa, camiseta ou vestido</small></p></div><div className={hasBottom?styles.done:''}><span>{hasBottom?'✓':'○'}</span><p><strong>Parte de baixo</strong><small>Calça, saia, short ou vestido</small></p></div><div className={hasShoes?styles.done:''}><span>{hasShoes?'✓':'○'}</span><p><strong>Calçado</strong><small>Tênis, sandália, salto ou sapato</small></p></div></div></div><button className={styles.savePieceButton} onClick={()=>setSheet('add')}>Adicionar próxima peça</button><button className={styles.secondarySheetButton} onClick={()=>{setSheet(null);setWardrobeOpen(true)}}>Abrir meu guarda-roupa</button></>}
