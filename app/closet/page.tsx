...
import { ClosetSession, getStoredSession, loadClosetItems, saveClosetItem, signInCloset, signOutCloset, signUpCloset } from './supabase';

type Category='Todos'|'Blusas'|'Calças'|'Vestidos'|'Calçados'|'Bolsas'|'Acessórios';
type Sheet='add'|'scan'|'scanReject'|'edit'|'occasion'|'wardrobe'|'closetStatus'|'auth'|null;
...
export default function ClosetPage(){
 ...
 const [session,setSession]=useState<ClosetSession|null>(null),[authMode,setAuthMode]=useState<'login'|'signup'>('login'),[authEmail,setAuthEmail]=useState(''),[authPassword,setAuthPassword]=useState(''),[authName,setAuthName]=useState(''),[authBusy,setAuthBusy]=useState(false),[saving,setSaving]=useState(false);
 ...
 useEffect(()=>{const s=getStoredSession();if(s){setSession(s);refreshCloset(s)}},[]);
 async function handleAuth(){
  if(!authEmail||!authPassword){notify('Preencha e-mail e senha.');return}
  setAuthBusy(true);
  try{
   if(authMode==='login'){
    const s=await signInCloset(authEmail.trim(),authPassword);setSession(s);await refreshCloset(s);notify('Closet conectado ✦');setSheet(current?'edit':null);
   }else{
    const result=await signUpCloset(authEmail.trim(),authPassword,authName.trim());
    if(result.session){setSession(result.session);await refreshCloset(result.session);notify('Conta criada ✦');setSheet(current?'edit':null)}
    else notify('Conta criada. Confirme seu e-mail para entrar.');
   }
  }catch(e:any){notify(e?.message||'Não consegui entrar.')}finally{setAuthBusy(false)}
 }
 function logout(){signOutCloset();setSession(null);setPieces([]);setSheet(null);notify('Você saiu do closet.');}
 ...
 async function saveCurrent(){
  if(!current)return;
  if(!session){setSheet('auth');notify('Entre ou crie sua conta para guardar a peça.');return}
  setSaving(true);
  try{
 ...
 const initials=session?.user?.user_metadata?.name?.split(' ').map((x:string)=>x[0]).join('').slice(0,2).toUpperCase()||session?.user?.email?.slice(0,2).toUpperCase()||'MC';
 return <main className={styles.page}><section className={styles.appShell}>
  <header className={styles.topbar}><div><span className={styles.eyebrow}>SEU ESTILO, TODOS OS DIAS</span><h1>closet<span>.</span></h1></div><button className={styles.profileButton} onClick={()=>setSheet('auth')}>{initials}</button></header>
 ...
 {sheet==='auth'&&<><div className={styles.sheetHeader}><div><span className={styles.kicker}>Sua conta</span><h3>{session?'Closet conectado':authMode==='login'?'Entre no seu closet':'Crie seu closet'}</h3></div><button onClick={()=>setSheet(null)}>×</button></div>{session?<><p className={styles.sheetIntro}>Seu guarda-roupa está sendo salvo na nuvem como <strong>{session.user.email}</strong>.</p><button className={scanStyles.secondaryAction} onClick={logout}>Sair da conta</button></>:<><p className={styles.sheetIntro}>Entre para que suas peças continuem aqui mesmo depois de fechar o navegador ou trocar de aparelho.</p><div className={styles.formGrid}>{authMode==='signup'&&<label>Nome<input value={authName} onChange={e=>setAuthName(e.target.value)} placeholder="Seu nome"/></label>}<label>E-mail<input type="email" value={authEmail} onChange={e=>setAuthEmail(e.target.value)} placeholder="voce@email.com"/></label><label>Senha<input type="password" value={authPassword} onChange={e=>setAuthPassword(e.target.value)} placeholder="mínimo 6 caracteres"/></label></div><button className={styles.savePieceButton} disabled={authBusy} onClick={handleAuth}>{authBusy?'Conectando...':authMode==='login'?'Entrar':'Criar conta'}</button><button className={scanStyles.secondaryAction} onClick={()=>setAuthMode(v=>v==='login'?'signup':'login')}>{authMode==='login'?'Ainda não tenho conta':'Já tenho uma conta'}</button></>}</>}
