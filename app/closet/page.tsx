...
export default function ClosetPage(){
 ...
 async function buildLook(occasion:string){
  const selected=selectLookPieces(pieces,occasion);setSelectedOccasion(occasion);setLookBuilding(true);setLookReady(false);setTryOnImage('');setSheet('buildingLook');
  await new Promise(resolve=>window.setTimeout(resolve,1650));
  setLookPieces(selected);setLookReady(true);setLookBuilding(false);setSheet(null);
  window.setTimeout(()=>document.getElementById('closet-look')?.scrollIntoView({behavior:'smooth',block:'start'}),120)
 }
 function remixLook(){
