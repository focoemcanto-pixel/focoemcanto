...
import { ClosetSession, loadClosetItems, restoreClosetSession, saveClosetItem, saveClosetItems } from './supabase';

type Category='Todos'|'Blusas'|'Calças'|'Vestidos'|'Calçados'|'Bolsas'|'Acessórios';
type Sheet='add'|'scan'|'scanReject'|'edit'|'occasion'|'wardrobe'|'closetStatus'|'auth'|'avatar'|'buildingLook'|null;
...
function pieceText(p:Piece){return `${p.name} ${p.meta}`.toLowerCase()}
function occasionScore(p:Piece,occasion:string){
 const t=pieceText(p),o=occasion.toLowerCase();let s=0;
 const formal=['social','alfaiataria','camisa','blazer','terno','sapato','cinto','relógio','relogio','vestido'];
 const casual=['tênis','tenis','camiseta','jeans','malha','suéter','sueter','sapatênis','sapatenis'];
 if(['evento','igreja','trabalho','encontro','festa'].includes(o))formal.forEach(k=>{if(t.includes(k))s+=4});
 if(['faculdade','escola','sair','viagem'].includes(o))casual.forEach(k=>{if(t.includes(k))s+=4});
 if(o==='evento'&&/(branco|preto|bege|azul|marrom|cinza|verde escuro)/.test(t))s+=2;
 if(o==='igreja'&&/(social|camisa|calça|sapato|cinto|relógio)/.test(t))s+=3;
 if(o==='trabalho'&&/(social|alfaiataria|camisa|calça|sapato|cinto)/.test(t))s+=3;
 if(o==='festa'&&/(acessório|oculos|óculos|relógio|cinto)/.test(t))s+=2;
 return s;
}
function bestOf(list:Piece[],occasion:string){return [...list].sort((a,b)=>occasionScore(b,occasion)-occasionScore(a,occasion))[0]}
function selectLookPieces(all:Piece[],occasion:string){
 const dresses=all.filter(p=>p.category==='Vestidos'),tops=all.filter(p=>p.category==='Blusas'),bottoms=all.filter(p=>p.category==='Calças'),shoes=all.filter(p=>p.category==='Calçados'),bags=all.filter(p=>p.category==='Bolsas'),acc=all.filter(p=>p.category==='Acessórios');
 const result:Piece[]=[];
 const dress=bestOf(dresses,occasion),top=bestOf(tops,occasion),bottom=bestOf(bottoms,occasion),shoe=bestOf(shoes,occasion);
 const useDress=Boolean(dress)&&(!top||!bottom||occasion==='Festa'||occasion==='Evento');
 if(useDress&&dress)result.push(dress);else{if(top)result.push(top);if(bottom)result.push(bottom)}
 if(shoe)result.push(shoe);
 const rankedAcc=[...acc,...bags].sort((a,b)=>occasionScore(b,occasion)-occasionScore(a,occasion));
 for(const p of rankedAcc){if(result.length>=6)break;const t=pieceText(p);if(result.some(x=>x.id===p.id))continue;if(result.some(x=>{const xt=pieceText(x);return (t.includes('óculos')&&xt.includes('óculos'))||(t.includes('relog')&&xt.includes('relog'))||(t.includes('cinto')&&xt.includes('cinto'))}))continue;result.push(p)}
 return result;
}
function lookSlot(p:Piece,index:number){
 if(p.category==='Vestidos')return lookStyles.dressPiece;
 if(p.category==='Blusas')return lookStyles.topPiece;
 if(p.category==='Calças')return lookStyles.bottomPiece;
 if(p.category==='Calçados')return lookStyles.shoePiece;
 const accessories=[lookStyles.accessoryLeft,lookStyles.accessoryRight,lookStyles.accessoryLowerLeft,lookStyles.accessoryLowerRight];
...
export default function ClosetPage(){
 ...
 const [lookPieces,setLookPieces]=useState<Piece[]>([]),[lookBuilding,setLookBuilding]=useState(false);
 const [swapPieceId,setSwapPieceId]=useState<string|number|null>(null),[swapReturnAvatar,setSwapReturnAvatar]=useState(false);
 ...
 const swapCandidates=useMemo(()=>selectedSlot==='Peça'?pieces:pieces.filter(p=>p.category===selectedSlot),[pieces,selectedSlot]);
 ...
 async function saveAll(){
  ...
  setSaving(true);
  try{
   const inputs=prepared.map((p,i)=>({name:i===currentIndex?(draftName||p.name):p.name,category:i===currentIndex?draftCategory:p.category,color:i===currentIndex?draftColor:p.color,source:p,catalogImage:p.image,originalImage:draftOriginal}));
   const rows=await saveClosetItems(session,inputs);
   const saved=mapDbRows(rows);
   setPieces(v=>[...saved,...v.filter(existing=>!saved.some(n=>n.id===existing.id))]);
   setSheet(null);setWardrobeOpen(true);notify(`${saved.length} peças guardadas de uma vez ✦`);setPrepared([]);setCurrentIndex(0)
  }catch(e:any){notify(e?.message||'Não consegui guardar todas as peças.')}finally{setSaving(false)}
 }
 async function generateTryOn(){
  if(!lookPieces.length)return;setTryOnBusy(true);setTryOnError('');setTryOnImage('');
  try{
   const image=await makeLookReference(lookPieces);
   const r=await fetch('/api/closet/try-on',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({image,avatarId,occasion:selectedOccasion,items:lookPieces.map(p=>({name:p.name,category:p.category,meta:p.meta}))})});
   const data=await r.json();
   if(!r.ok||!data?.ok||!data?.image)throw new Error(data?.message||'Não consegui vestir este look.');
   setTryOnImage(String(data.image));
  }catch(e:any){const raw=String(e?.message||'');setTryOnError(/credits|billing|quota/i.test(raw)?'A geração visual está temporariamente sem créditos de API. O restante do closet continua funcionando normalmente.':raw||'Não consegui vestir este look agora.')}finally{setTryOnBusy(false)}
 }
 async function buildLook(occasion:string){
  setSelectedOccasion(occasion);setLookBuilding(true);setSheet('buildingLook');
  sessionStorage.setItem('closet_last_occasion',occasion);
  await new Promise(resolve=>window.setTimeout(resolve,720));
  window.location.href=`/closet/look?occasion=${encodeURIComponent(occasion)}`;
 }
 function remixLook(){
  const used=new Set<string|number>();
  const next=lookPieces.map(current=>{const options=pieces.filter(p=>p.category===current.category&&p.id!==current.id&&!used.has(p.id)).sort((a,b)=>occasionScore(b,selectedOccasion)-occasionScore(a,selectedOccasion));const chosen=options[0]||current;used.add(chosen.id);return chosen});
  setLookPieces(next);setTryOnImage('');setTryOnError('');notify('Novo look sorteado ✦')
 }
 function startLook(){if(!readyForLook){setSheet('closetStatus');return}setSheet('occasion')}
 function openSwap(s:string,id:string|number|null=null,returnAvatar=false){setSelectedSlot(s);setSwapPieceId(id);setSwapReturnAvatar(returnAvatar);setSheet('wardrobe')}
 function applySwap(candidate:Piece){
  setLookPieces(currentLook=>{let replaced=false;return currentLook.map(p=>{if((swapPieceId!==null&&p.id===swapPieceId)||(swapPieceId===null&&!replaced&&(selectedSlot==='Peça'||p.category===candidate.category))){replaced=true;return candidate}return p})});
  setTryOnImage('');setTryOnError('');notify(`${candidate.name} entrou no look ✦`);setSheet(swapReturnAvatar?'avatar':null)
 }
 ...
 return <main className={styles.page}><section className={styles.appShell}>
  ...
  <section id="closet-look" className={styles.lookSection}><div className={styles.sectionHeader}><div><span className={styles.kicker}>Seu look</span><h3>{lookReady?`Para ${selectedOccasion}`:'Pronto quando você estiver'}</h3></div></div>{!lookReady?<div className={styles.lookPreviewEmpty}><span>✦</span><h4>{readyForLook?'Seu stylist já pode começar.':'Seu primeiro look está quase pronto.'}</h4><p>{readyForLook?'Escolha uma ocasião e eu monto usando o seu closet.':'Basta uma parte de cima, uma parte de baixo e um calçado para testar.'}</p><button onClick={startLook}>{readyForLook?'Montar meu look':'Ver o que falta'}</button></div>:<div className={styles.lookCard}><div className={`${styles.lookCanvas} ${lookStyles.lookWardrobeStage}`}><i className={`${lookStyles.wardrobeSide} ${lookStyles.sideLeft}`}/><i className={`${lookStyles.wardrobeSide} ${lookStyles.sideRight}`}/>{lookPieces.map((p,i)=><button key={p.id} className={`${lookStyles.lookPiece} ${lookSlot(p,i)}`} style={{animationDelay:`${Math.min(i,5)*90}ms`}} onClick={()=>openSwap(p.category,p.id,false)}><img src={p.image} alt={p.name}/></button>)}<span className={lookStyles.lookCaption}>✦ look montado para {selectedOccasion.toLowerCase()}</span></div><div className={styles.lookDetails}><div className={lookStyles.lookSummary}>{lookPieces.map(p=><button className={lookStyles.lookSummaryButton} key={p.id} onClick={()=>openSwap(p.category,p.id,false)}>{p.name}</button>)}</div><p>Montei um conjunto completo pensando em <strong>{selectedOccasion.toLowerCase()}</strong>, usando primeiro as peças essenciais e depois os acessórios que fazem sentido.</p><div className={styles.lookActions}><button onClick={remixLook}>⤨ Sortear outro look</button><button onClick={()=>openSwap('Peça',null,false)}>↻ Trocar uma peça</button><button onClick={()=>setWardrobeOpen(true)}>▤ Abrir guarda-roupa</button><button onClick={()=>{setTryOnError('');setSheet('avatar')}}>♙ Ver vestido</button><button className={liked?styles.liked:''} onClick={()=>setLiked(v=>!v)}>{liked?'♥':'♡'} {liked?'Amei':'Gostei'}</button></div></div></div>}</section>
  <nav className={styles.bottomNav}><button className={styles.navActive}><span>⌂</span>Início</button><button onClick={()=>setWardrobeOpen(true)}><span>♢</span>Closet</button><button className={styles.fab} onClick={startLook}>✦</button><button onClick={()=>window.location.href='/closet/look'}><span>▦</span>Looks</button><button><span>◌</span>Loja</button></nav>
 </section>
 ...
 {sheet&&<button className={styles.scrim} onClick={()=>sheet!=='buildingLook'&&setSheet(null)}/>}<aside className={`${styles.sheet} ${sheet?styles.sheetOpen:''}`}><div className={styles.sheetHandle}/>
 ...
 {sheet==='edit'&&current&&<><div className={styles.sheetHeader}><div><span className={styles.kicker}>{prepared.length>1?`${prepared.length} peças identificadas · ${currentIndex+1} de ${prepared.length}`:'Peça de catálogo pronta'}</span><h3>{currentIndex===0?'Confira e guarde':'Próxima peça'}</h3></div><button onClick={()=>setSheet(null)}>×</button></div>{prepared.length>1&&<div style={{display:'flex',gap:8,overflowX:'auto',padding:'8px 0 12px'}}>{prepared.map((p,i)=><button key={i} onClick={()=>{setCurrentIndex(i);loadDraft(i)}} style={{flex:'0 0 72px',height:86,border:i===currentIndex?'2px solid #211b17':'1px solid #ddd0c2',borderRadius:14,background:'#f4ecdf',padding:5}}><img src={p.image} alt={p.name} style={{width:'100%',height:'100%',objectFit:'contain'}}/></button>)}</div>}<div className={`${styles.editorPreview} ${scanStyles.transparentPreview}`}><img src={current.image} alt={current.name}/><span className={scanStyles.detectedBadge}>✓ versão de catálogo</span></div><div className={styles.formGrid}><label>Nome<input value={draftName} onChange={e=>setDraftName(e.target.value)}/></label><label>Categoria<select value={draftCategory} onChange={e=>setDraftCategory(e.target.value as Category)}>{categories.filter(c=>c!=='Todos').map(c=><option key={c}>{c}</option>)}</select></label><label>Cor principal<input value={draftColor} onChange={e=>setDraftColor(e.target.value)}/></label></div>{prepared.length>1&&<button className={styles.savePieceButton} disabled={saving} onClick={saveAll}>{saving?'Guardando todas...':`Guardar todas as ${prepared.length} peças`}</button>}<button className={prepared.length>1?scanStyles.secondaryAction:styles.savePieceButton} disabled={saving} onClick={saveCurrent}>{saving?'Guardando...':currentIndex<prepared.length-1?'Guardar só esta e ver próxima →':'Guardar no meu closet'}</button><button className={scanStyles.secondaryAction} onClick={()=>setSheet('add')}>Usar outra foto</button></>}
 ...
 {sheet==='buildingLook'&&<><div className={styles.sheetHeader}><div><span className={styles.kicker}>Stylist trabalhando</span><h3>Montando seu look...</h3></div></div><div className={lookStyles.buildStage}><div className={lookStyles.buildWardrobe}><div className={lookStyles.buildInside}><span className={lookStyles.buildSpark}>✦</span><strong>Escolhendo a combinação</strong><small>Primeiro roupa, depois calçado e acessórios — tudo pensando em {selectedOccasion.toLowerCase()}.</small></div><i className={`${lookStyles.buildDoor} ${lookStyles.buildLeft}`}/><i className={`${lookStyles.buildDoor} ${lookStyles.buildRight}`}/></div><div className={lookStyles.buildCopy}><strong>Abrindo seu closet...</strong><p>Seu look vai abrir em uma tela própria.</p></div></div></>}
 {sheet==='avatar'&&<><div className={styles.sheetHeader}><div><span className={styles.kicker}>Provador do look</span><h3>Abra a cortina e veja vestido</h3></div><button onClick={()=>setSheet(null)}>×</button></div><p className={styles.sheetIntro}>Escolha o manequim, abra a cortina e veja o look completo. Toque em qualquer peça abaixo para trocar sem refazer o restante.</p><TryOnStage pieces={lookPieces} models={avatarModels} avatarId={avatarId} image={tryOnImage} busy={tryOnBusy} error={tryOnError} onAvatarChange={id=>{setAvatarId(id);setTryOnImage('');setTryOnError('')}} onGenerate={generateTryOn} onSwap={p=>openSwap(p.category,p.id,true)} onRemix={remixLook} onResetModel={()=>{setTryOnImage('');setTryOnError('')}}/></>}
 ...
 {sheet==='occasion'&&<><div className={styles.sheetHeader}><div><span className={styles.kicker}>Montar look</span><h3>Para onde você vai?</h3></div><button onClick={()=>setSheet(null)}>×</button></div><div className={styles.occasionGrid}>{occasions.map(([n,d])=><button key={n} className={selectedOccasion===n?styles.occasionActive:''} onClick={()=>setSelectedOccasion(n)}><strong>{n}</strong><small>{d}</small></button>)}</div><button className={styles.savePieceButton} disabled={!selectedOccasion||lookBuilding} onClick={()=>buildLook(selectedOccasion)}>{lookBuilding?'Montando...':'✦ Montar meu look'}</button></>}
 {sheet==='wardrobe'&&<><div className={styles.sheetHeader}><div><span className={styles.kicker}>Trocar peça</span><h3>{selectedSlot==='Peça'?'Escolha uma substituição':selectedSlot}</h3></div><button onClick={()=>setSheet(swapReturnAvatar?'avatar':null)}>×</button></div><p className={styles.sheetIntro}>Apenas esta parte do look será alterada.</p><div className={styles.swapGrid}>{swapCandidates.filter(p=>p.id!==swapPieceId).slice(0,12).map(p=><button key={p.id} onClick={()=>applySwap(p)}><img src={p.image} alt={p.name}/><strong>{p.name}</strong><small>{p.meta}</small></button>)}</div></>}
