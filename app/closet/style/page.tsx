...
function buildRoutines(ids:string[],work:{profession:string;dress_code:string;notes:string},existing:any[]=[]){
 const old=new Map((Array.isArray(existing)?existing:[]).map(x=>[String(x.id),x]));
 const defaults:Record<string,any>={
  trabalho:{id:'trabalho',title:'Trabalho',occasion:'Trabalho',detail:'Dia normal',period:'auto',paletteMode:'auto'},
  igreja:{id:'igreja',title:'Igreja',occasion:'Igreja',detail:'Culto comum',period:'auto',paletteMode:'auto'},
  aula:{id:'aula',title:'Aula',occasion:'Aula',detail:'Aula comum',period:'auto',paletteMode:'auto'},
  eventos:{id:'eventos',title:'Eventos',occasion:'Evento',detail:'Casual arrumado',period:'auto',paletteMode:'auto'},
  lazer:{id:'lazer',title:'Dia a dia',occasion:'Passeio',detail:'Passeio casual',period:'auto',paletteMode:'auto'},
  academia:{id:'academia',title:'Academia',occasion:'Academia',detail:'Treino',period:'auto',paletteMode:'auto'}
 };
 return ids.map(id=>({...defaults[id],...(old.get(id)||{}),id,title:(old.get(id) as any)?.title||defaults[id]?.title||id,...(id==='trabalho'?{work_profile:work}:{})}));
}
...
export default function StylePage(){
 ...
 return <main className={styles.page}>
  ...
  {loading?<div className={styles.loading}>Conhecendo seu estilo…</div>:<section className={styles.content}>
   ...
   <div className={styles.block}><span>SUA ROTINA</span><p className={styles.blockText}>Esses contextos viram atalhos de montagem rápida. Você não precisará responder tudo novamente cada vez que for se vestir.</p><div className={styles.chips}>{routineOptions.map(([id,label])=><button key={id} className={routine.includes(id)?styles.activeChip:''} onClick={()=>toggle(routine,setRoutine,id,8)}>{label}</button>)}</div>{routine.length>0&&<div className={styles.routineDefaults}>{routine.map(id=>{const r=routineValue(id);return <div className={styles.routineDefaultCard} key={id}><div><strong>{r.title}</strong><small>Defina o padrão. No dia a dia você só altera se precisar.</small></div><label>Contexto<select value={r.detail||''} onChange={e=>changeRoutine(id,{detail:e.target.value})}>{(routineDetails[id]||['Casual']).map(x=><option key={x} value={x}>{x}</option>)}</select></label><label>Horário<select value={r.period||'auto'} onChange={e=>changeRoutine(id,{period:e.target.value})}><option value="auto">Usar horário atual</option><option value="manha">Manhã</option><option value="tarde">Tarde</option><option value="noite">Noite</option></select></label></div>})}</div>}</div>
