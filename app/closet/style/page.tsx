...
export default function StylePage(){
 const [session,setSession]=useState<ClosetSession|null>(null),[loading,setLoading]=useState(true),[saving,setSaving]=useState(false),[stylesPicked,setStylesPicked]=useState<string[]>([]),[preferredColors,setPreferredColors]=useState<string[]>([]),[avoidColors,setAvoidColors]=useState<string[]>([]),[fit,setFit]=useState('regular'),[boldness,setBoldness]=useState(.45),[toast,setToast]=useState('');
 function notify(m:string){setToast(m);window.setTimeout(()=>setToast(''),2200)}
 useEffect(()=>{let alive=true;(async()=>{const s=await restoreClosetSession();if(!alive)return;if(!s){setLoading(false);return}setSession(s);const p:any=await loadStyleProfile(s);if(!alive)return;setStylesPicked(p?.preferred_styles||p?.styles||[]);setPreferredColors(p?.preferred_colors||p?.colors||[]);setAvoidColors(p?.avoid_colors||[]);setFit(p?.fit||'regular');setBoldness(Number(p?.boldness??.45));setLoading(false)})().catch(()=>setLoading(false));return()=>{alive=false}},[]);
 ...
 async function save(){if(!session)return;setSaving(true);try{await saveStyleProfile(session,{preferred_styles:stylesPicked,styles:stylesPicked,preferred_colors:preferredColors,colors:preferredColors,avoid_colors:avoidColors,fit,boldness,formality:stylesPicked.includes('clássico')?.65:.4,profile_version:1},true);notify('Seu DNA de estilo foi salvo ✦');window.setTimeout(()=>window.location.href='/closet/look',500)}catch(e:any){notify(e?.message||'Não consegui salvar seu estilo.')}finally{setSaving(false)}}
 ...
 return <main className={styles.page}>
  ...
  {loading?<div className={styles.loading}>Conhecendo seu estilo…</div>:<section className={styles.content}>
   <div className={styles.hero}><span>PASSO 1</span><h1>Qual visual mais parece com você?</h1><p>Escolha até quatro linguagens. O stylist mistura isso com ocasião, clima e seu próprio guarda-roupa.</p><div className={styles.dna}>{dna}</div></div>
   ...
   <button className={styles.save} disabled={saving||!stylesPicked.length} onClick={save}>{saving?'Salvando seu DNA…':'Salvar e personalizar meus looks'}</button>
  </section>}
