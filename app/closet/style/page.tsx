...
export default function StylePage(){
 ...
 return <main className={styles.page}>
  ...
  {loading?<div className={styles.loading}>Conhecendo seu estilo…</div>:<section className={styles.content}>
   ...
   <div className={styles.block}><span>SUA ROTINA</span><p className={styles.blockText}>Esses contextos viram atalhos de montagem rápida. Você não precisará responder tudo novamente cada vez que for se vestir.</p><div className={styles.chips}>{routineOptions.map(([id,label])=><button key={id} className={routine.includes(id)?styles.activeChip:''} onClick={()=>toggle(routine,setRoutine,id,8)}>{label}</button>)}</div></div>
