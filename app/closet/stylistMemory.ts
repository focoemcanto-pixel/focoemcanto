...
let liveMemory:StylistMemory|null=null;
...
function saveMemory(session:ClosetSession,m:StylistMemory){m.updatedAt=new Date().toISOString();liveMemory=m;localStorage.setItem(key(session.user.id),JSON.stringify(m));return m}
export function latestStylistMemory(){return liveMemory}
...
export function loadStylistMemory(session:ClosetSession):StylistMemory{try{const current=JSON.parse(localStorage.getItem(key(session.user.id))||'null');if(current){liveMemory={...empty(),...current,contextLikes:current.contextLikes||{},tagLikes:current.tagLikes||{}};return liveMemory}const legacy=JSON.parse(localStorage.getItem(oldKey(session.user.id))||'null');if(legacy){const migrated={...empty(),...legacy,contextLikes:{},tagLikes:{}};saveMemory(session,migrated);return migrated}liveMemory=empty();return liveMemory}catch{liveMemory=empty();return liveMemory}}
