...
export default function TryOnStage({pieces,models,avatarId,image,busy,error,onAvatarChange,onGenerate,onSwap,onRemix,onResetModel,onClose}:Props){
 ...
 return <div className={lookStyles.lookExperience}>
  ...
  <div className={lookStyles.lookExperienceIntro}><span>VISUALIZAÇÃO DO LOOK</span><h2>{open?'Seu look está pronto.':'Veja a combinação no provador.'}</h2><p>{busy?'Estou vestindo exatamente as peças escolhidas. A cortina abre assim que estiver pronto.':'Escolha um manequim e abra a cortina. Depois você pode trocar uma peça ou sortear outra combinação.'}</p></div>
  <div className={lookStyles.fittingRoomWrap}>
   ...
   <div className={lookStyles.pieceRail}>{pieces.map(p=><button key={p.id} onClick={()=>onSwap(p)}><img src={p.image} alt={p.name}/><span>{p.name}</span><small>Toque para trocar</small></button>)}</div>
   ...
   <div className={lookStyles.fittingActions}>{!image?<button className={lookStyles.primaryStageButton} disabled={busy} onClick={onGenerate}>{busy?'Vestindo atrás da cortina...':'✦ Abrir cortina e ver vestido'}</button>:<button className={lookStyles.primaryStageButton} onClick={onResetModel}>Ver em outro modelo</button>}<button onClick={onRemix}>⤨ Sortear outro look</button><button onClick={()=>pieces[0]&&onSwap(pieces[0])}>↻ Trocar uma peça</button></div>
  </div>
 </div>
}
