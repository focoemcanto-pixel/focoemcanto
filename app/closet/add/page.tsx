...
export default function AddPiecePage(){
  ...
  const [mode,setMode]=useState<'choose'|'manual'|'ai-photo'|'ai-results'>('choose');
  ...
  const selectedCount=useMemo(()=>detected.filter(x=>x.selected).length,[detected]);
  ...
  async function filesToDrafts(files:File[]){
    ...
    return images.map((photo,i):ManualDraft=>({id:crypto.randomUUID(),image:photo,name:'',category:'Blusas',color:'',subcategory:''}));
  }
  ...
  async function receiveAi(e:ChangeEvent<HTMLInputElement>){
    ...
    let photo='';
    try{photo=await imageToDataUrl(f)}catch{setMessage('Não consegui abrir esta foto.');return}
    if(!session){setImage(photo);requireAuth('ai');return}
    if((wallet?.balance||0)<1){
      const draft:ManualDraft={id:crypto.randomUUID(),image:photo,name:'',category:'Blusas',color:'',subcategory:''};
    ...
    try{
      setImage(photo);
      const data=await apiJson('/api/closet/scan',session,{image:photo}),items=(data?.scan?.items||[]) as ScanItem[];
      if(!items.length)throw new Error(data?.scan?.reason||'Não encontrei peças suficientemente visíveis.');
      const ready=await Promise.all(items.map(async item=>({...item,crop:await cropByBox(photo,item.box),selected:true})));
  ...
  async function saveDetected(item:Detected,index:number,useAi:boolean){
    ...
    try{
      let asset=item.crop;
      if(useAi){if((wallet?.balance||0)<1)throw Object.assign(new Error('Você precisa de 1 crédito para melhorar esta peça.'),{code:'insufficient_ai_credits'});const data=await apiJson('/api/closet/catalogize',session,{image:item.crop,item});asset=String(data.image);setDetected(v=>v.map((x,i)=>i===index?{...x,catalog:asset}:x));await refreshWallet()}
  ...
  return <main className={styles.page}>
    ...
    {mode==='choose'?<section className={styles.choose}>
      ...
      <button className={styles.aiCard} onClick={()=>session?setMode('ai-photo'):requireAuth('ai')}><b>CLOSET AI</b><strong>Scanner inteligente</strong><small>Identifica as peças de uma foto. O scanner custa 1 crédito. Depois você decide, peça por peça, se quer melhorar a imagem por mais 1 crédito.</small><i>{wallet?`${wallet.balance} créditos disponíveis`:session?'Carregando saldo':'Entrar e ver saldo'} →</i></button>
      <div className={styles.note}>Fotos da galeria nunca acionam IA sozinhas. Sem saldo, a foto permanece disponível para cadastro gratuito.</div>
      ...
      {!manualDrafts.length?<div className={styles.capture}>
        ...
        <div className={styles.form}>
          ...
          <label>Subcategoria <small>opcional</small><input value={activeDraft.subcategory} onChange={e=>patchDraft({subcategory:e.target.value})} placeholder="Ex.: camisa social, tênis, chino"/></label>
          <label>Cor principal <small>opcional</small><input value={activeDraft.color} onChange={e=>patchDraft({color:e.target.value})} placeholder="Ex.: off-white"/></label>
        </div>
        {message&&<div className={styles.message}>{message}</div>}
        <button className={styles.save} disabled={saving} onClick={saveManual}>{saving?'Guardando…':session?(manualDrafts.length>1?'Guardar esta e continuar':'Guardar no meu closet'):'Entrar e guardar no meu closet'}</button>
        {manualDrafts.length>1&&<div className={styles.note}>As fotos ficam nesta fila até você salvar ou remover. Nenhuma delas será enviada ao scanner automaticamente.</div>}
      </>}
    </section>:mode==='ai-photo'?<section className={styles.manual}>
      <div className={styles.title}><span>CLOSET AI · 1 CRÉDITO</span><h1>Escaneie uma foto.</h1><p>O scanner só roda depois desta escolha explícita. Se seu saldo estiver zerado, a foto será mantida e aberta no cadastro gratuito.</p></div>
      <div className={styles.balance}><span>Seu saldo</span><strong>{wallet?.balance??0} créditos</strong><a href="/closet/credits">Ver carteira</a></div>
      {message&&<div className={styles.message}>{message}</div>}
      <div className={styles.capture}><button disabled={busy} onClick={()=>aiCamera.current?.click()}><span>◎</span><strong>{busy?'Analisando…':'Tirar foto'}</strong><small>usa 1 crédito</small></button><button disabled={busy} onClick={()=>aiGallery.current?.click()}><span>▧</span><strong>{busy?'Analisando…':'Escolher foto'}</strong><small>usa 1 crédito</small></button></div>
      <button className={styles.aiLater} onClick={()=>setMode('choose')}>← Voltar sem gastar</button>
    </section>:<section className={styles.results}>
      <div className={styles.title}><span>PEÇAS ENCONTRADAS</span><h1>{detected.length} {detected.length===1?'peça identificada':'peças identificadas'}.</h1><p>O scanner já foi concluído. Agora nenhuma outra IA roda sem você pedir.</p></div>
      ...
      {message&&<div className={styles.message}>{message}</div>}
      {selectedCount>0&&<button className={styles.saveAllFree} disabled={busy} onClick={saveAllFree}>Guardar todas com os recortes · grátis</button>}
      <div className={styles.detectedGrid}>{detected.map((item,i)=><article key={`${item.name}-${i}`} className={!item.selected?styles.done:''}><div className={styles.detectedImage}><img src={item.catalog||item.crop} alt={item.name}/>{!item.selected&&<span>✓ guardada</span>}</div><div className={styles.detectedCopy}><small>{item.category} · {item.color}</small><strong>{item.name}</strong><p>{item.subcategory}</p>{item.selected&&<div className={styles.itemActions}><button disabled={busy} onClick={()=>saveDetected(item,i,false)}>Guardar com esta foto <em>grátis</em></button><button disabled={busy||(wallet?.balance||0)<1} className={styles.itemAi} onClick={()=>saveDetected(item,i,true)}>Melhorar e guardar <em>1 crédito</em></button></div>}</div></article>)}</div>
      <div className={styles.resultFooter}><strong>{selectedCount?`${selectedCount} ainda não guardada${selectedCount>1?'s':''}`:'Tudo guardado'}</strong><button onClick={()=>location.href='/closet/wardrobe'}>Ir para meu closet →</button></div>
    </section>}
