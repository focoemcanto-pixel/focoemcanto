...
export default function SellerLayout({children}:{children:React.ReactNode}){const p=usePathname();return <div className={styles.sellerBody}>{children}<nav className={styles.nav} aria-label="Operação do vendedor">{items.map(([href,label,path])=>{const active=href==='/closet/seller'?p===href||p===href+'/':p.startsWith(href);return <a key={href} href={href} aria-current={active?'page':undefined} aria-label={label} className={`${styles.item} ${active?styles.active:''}`}>{ico(path)}<span>{label}</span></a>})}</nav></div>}
