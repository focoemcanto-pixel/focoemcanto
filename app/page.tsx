...
export default function Home() {
  return (
    <main>
      ...
      <section className="hero-section hero-vsl-section">
        <div className="container hero-vsl-container">
        ...
        <div className="container">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.5rem', alignItems: 'stretch' }}>
            <article style={{ background: 'linear-gradient(145deg,#18181b,#0b0b0c)', border: '1px solid rgba(255,255,255,.14)', borderRadius: '28px', padding: 'clamp(1.6rem,4vw,2.2rem)', boxShadow: '0 18px 60px rgba(0,0,0,.32)', display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ marginTop: 'auto', paddingTop: '1.5rem', borderTop: '1px solid rgba(255,255,255,.12)' }}><span style={{ display: 'block', color: 'rgba(255,255,255,.55)', fontWeight: 700 }}>comece por</span><strong style={{ display: 'block', fontSize: 'clamp(2.2rem,4vw,3.2rem)', lineHeight: 1, margin: '.5rem 0', letterSpacing: '-.05em' }}>12x R$30,72</strong><small style={{ display: 'block', color: 'rgba(255,255,255,.55)', fontWeight: 700 }}>ou <b style={{ color: '#fff' }}>R$297 à vista</b></small><span style={{ display: 'inline-flex', width: 'fit-content', marginTop: '.75rem', padding: '.45rem .75rem', borderRadius: '999px', background: 'rgba(244,200,75,.14)', color: 'var(--gold)', fontWeight: 900, fontSize: '.78rem' }}>à vista com desconto no Pix ou crédito</span></div>
              <a href={essentialCheckoutUrl} className="btn" style={{ width: '100%', marginTop: '1.6rem', padding: '1.25rem 1rem', borderRadius: '14px' }}>COMEÇAR PELO ESSENTIAL</a>
            </article>
            <article style={{ position: 'relative', background: 'radial-gradient(circle at top right,rgba(40,215,191,.22),transparent 34%),linear-gradient(145deg,#1d1d20,#090909)', border: '1.5px solid var(--accent)', borderRadius: '28px', padding: 'clamp(1.6rem,4vw,2.2rem)', boxShadow: '0 18px 70px rgba(40,215,191,.14)', display: 'flex', flexDirection: 'column' }}>
              <div style={{ display: 'inline-flex', width: 'fit-content', marginBottom: '1.4rem', background: 'linear-gradient(135deg,var(--accent),#2c8e5a)', color: '#fff', padding: '.7rem 1.2rem', borderRadius: '999px', fontWeight: 900, fontSize: '.82rem', boxShadow: '0 14px 34px rgba(40,215,191,.22)' }}>⭐ EXPERIÊNCIA COMPLETA</div><span style={{ color: 'var(--accent)', fontWeight: 900, fontSize: '.78rem', letterSpacing: '.12em', marginBottom: '.8rem' }}>PREMIUM</span><h3 style={{ fontSize: 'clamp(1.55rem,2.4vw,2rem)' }}>Foco em Canto Premium</h3><p style={{ color: 'rgba(255,255,255,.68)' }}>Ideal para quem deseja acompanhamento mais próximo, ambiente exclusivo e uma jornada mais direcionada.</p>
              <ul style={{ listStyle: 'none', display: 'grid', gap: '.9rem', margin: '1rem 0 2rem' }}><PlanCheck>Tudo do Essential</PlanCheck><PlanCheck>Grupo exclusivo de alunos</PlanCheck><PlanCheck>Sala virtual da mentoria</PlanCheck><PlanCheck>Prioridade nas dúvidas</PlanCheck><PlanCheck>Acompanhamento premium</PlanCheck><PlanCheck>15 dias de garantia</PlanCheck></ul>
              <div style={{ marginTop: 'auto', paddingTop: '1.5rem', borderTop: '1px solid rgba(255,255,255,.12)' }}><span style={{ display: 'block', color: 'rgba(255,255,255,.55)', fontWeight: 700 }}>experiência completa por</span><strong style={{ display: 'block', color: 'var(--accent)', fontSize: 'clamp(2.2rem,4vw,3.2rem)', lineHeight: 1, margin: '.5rem 0', letterSpacing: '-.05em' }}>12x R$72,09</strong><small style={{ display: 'block', color: 'rgba(255,255,255,.55)', fontWeight: 700 }}>ou <b style={{ color: '#fff' }}>R$697 à vista</b></small><span style={{ display: 'inline-flex', width: 'fit-content', marginTop: '.75rem', padding: '.45rem .75rem', borderRadius: '999px', background: 'rgba(244,200,75,.14)', color: 'var(--gold)', fontWeight: 900, fontSize: '.78rem' }}>à vista com desconto no Pix ou crédito</span></div>
              <a href={premiumCheckoutUrl} className="btn" style={{ width: '100%', marginTop: '1.6rem', padding: '1.25rem 1rem', borderRadius: '14px' }}>QUERO A EXPERIÊNCIA COMPLETA</a>
            </article>
          </div>
          <p className="renew-note" style={{ textAlign: 'center' }}>A assinatura é anual, com renovação automática no mesmo valor contratado.</p>
        </div>
      </section>
