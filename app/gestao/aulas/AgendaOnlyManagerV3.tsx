...
export default function AgendaOnlyManagerV3(){
  ...
  const opportunities=useMemo<Opportunity[]>(()=>{
    ...
    ;[...demand.values()].filter(x=>x.count>=2).sort((a,b)=>b.count-a.count).slice(0,2).forEach(x=>{
      const [d,p]=x.label.split('-').map(v=>v.trim());const has=filteredOpen.some(s=>norm(s.day)===norm(d)&&norm(period(s.time))===norm(p));if(!has)out.push({kind:'demand',title:`${x.count} pessoas procuram ${x.label}`,detail:'Não existe vaga aberta nesse período.',day:d})
    })

    days.forEach(d=>{
      const dayRows=filteredOccurrences.filter(o=>o.day===d).sort((a,b)=>minutes(a.time)-minutes(b.time))
      for(let i=0;i<dayRows.length-1;i++){
        const a=dayRows[i],b=dayRows[i+1]
        const free=minutes(b.time)-(minutes(a.time)+a.durationMinutes)
        const aPres=norm(a.student.modality)==='presencial',bPres=norm(b.student.modality)==='presencial'
        const different=aPres&&bPres&&norm(a.student.neighborhood)!==norm(b.student.neighborhood)&&a.student.neighborhood&&b.student.neighborhood
        if(different&&free<30)out.push({kind:'route',title:`Deslocamento apertado em ${d}`,detail:`${a.student.name||'Aluno'} (${a.student.neighborhood}) termina ${endTime(a.time,a.durationMinutes)} → ${b.student.name||'Aluno'} (${b.student.neighborhood}) às ${b.time}, com só ${Math.max(0,free)} min livres.`,day:d})
        else if(free>=90)out.push({kind:'gap',title:`Janela ociosa de ${Math.floor(free/60)}h${free%60?`${free%60}min`:''} em ${d}`,detail:`Entre ${a.student.name||'aluno'} (termina ${endTime(a.time,a.durationMinutes)}) e ${b.student.name||'aluno'} (${b.time}).`,day:d})
      }
    })
  ...
  const items=[
    ...filteredOccurrences.filter(o=>o.day===day).map(o=>({kind:'student' as const,time:o.time,occurrence:o})),
    ...filteredOpen.filter(s=>s.day===day).map(s=>({kind:'slot' as const,time:text(s.time),slot:s}))
    ...
    <div className={styles.shell}>
      ...
      {loading?<div className={styles.loading}>Analisando sua agenda...</div>:error?<div className={styles.error}>{error}</div>:<>
        <section className={styles.opportunityPanel}>
          <div className={styles.opportunityHead}><div><span>ASSISTENTE DE AGENDA</span><h2>Oportunidades</h2><p>O sistema cruza duração real das aulas, vagas, interessados e logística para mostrar onde agir primeiro.</p></div><div className={styles.opportunityCount}>{opportunities.length}</div></div>
        ...
        <div style={{display:'flex',justifyContent:'space-between',gap:12,flexWrap:'wrap',marginBottom:14}}>
          <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>{days.map((d,i)=><button key={d} onClick={()=>setDay(d)} style={{padding:'10px 14px',borderRadius:10,border:'1px solid rgba(255,255,255,.1)',background:day===d?'#efcc89':'#12171d',color:day===d?'#17120b':'#aab2bc',fontWeight:800,cursor:'pointer'}}>{short[i]}</button>)}</div>
          <div style={{display:'flex',gap:6,padding:4,borderRadius:12,background:'#0f141a',border:'1px solid rgba(255,255,255,.08)'}}>{(['Todas','Online','Presencial'] as ModalityFilter[]).map(m=><button key={m} onClick={()=>setModalityFilter(m)} style={{padding:'8px 12px',border:0,borderRadius:9,background:modalityFilter===m?'#242d38':'transparent',color:modalityFilter===m?'#fff':'#89939f',fontWeight:800,cursor:'pointer'}}>{m}</button>)}</div>
        </div>
        <section className={styles.agenda} style={{gridTemplateColumns:'1fr'}}><article className={styles.day}><div className={styles.dayHead}><h2>{day}</h2><span>{items.length} item{items.length===1?'':'s'} • {modalityFilter}</span></div><div className={styles.events}>{items.map(item=>item.kind==='student'?<a href="/gestao/aulas/alunos/" key={`s-${item.occurrence.key}`} className={styles.event} style={{background:text(item.occurrence.student.color)||'#7427b9',textDecoration:'none'}}><div className={styles.time}><span style={{display:'block'}}>{item.time||'--:--'}</span><small style={{fontSize:10,opacity:.72}}>até {endTime(item.time,item.occurrence.durationMinutes)}</small></div><div className={styles.eventBody}><strong>{text(item.occurrence.student.name)||'Aluno'}</strong><span>{text(item.occurrence.student.modality)||'Online'} • {durationLabel(item.occurrence.durationMinutes)}{item.occurrence.student.neighborhood?` • ${item.occurrence.student.neighborhood}`:''}</span><small>{text(item.occurrence.student.address)}</small></div><div className={styles.chevron}>›</div></a>:(()=>{const r=ranked(item.slot);return <button key={`v-${item.slot.id}`} className={`${styles.event} ${item.slot.status==='available'?styles.vacancy:''}`} style={item.slot.status==='occupied'?{background:'#222832'}:item.slot.status==='blocked'?{background:'#181b20'}:undefined} onClick={()=>inspect(item.slot)}><div className={styles.time}><span style={{display:'block'}}>{item.time}</span><small style={{fontSize:10,opacity:.72}}>até {endTime(item.time,item.slot.durationMinutes)}</small></div><div className={styles.eventBody}><strong>{item.slot.status==='available'?'Vaga disponível':item.slot.status==='occupied'?'Horário ocupado':'Horário bloqueado'}</strong><span>{item.slot.modality} • {durationLabel(item.slot.durationMinutes)}</span><small>{item.slot.status==='available'?(r.length?`${r.length} candidato${r.length>1?'s':''} • melhor ${r[0].score}%`:'Nenhum candidato compatível'):'Clique para gerenciar'}</small></div><div className={styles.chevron}>›</div></button>})())}{!items.length&&<div className={styles.emptyDay}>Nenhuma aula ou vaga neste dia com esse filtro.</div>}</div></article></section>
      </>}
    </div>
