...
type Lead={id?:string;status?:string;name?:string;whatsapp?:string;modality?:string;availability?:unknown;flexible?:boolean;startIntent?:string;goal?:string;neighborhood?:string;city?:string;createdAt?:string;updatedAt?:string}
...
function compatibility(lead:Lead,slot:Slot){
  ...
  for(const raw of listAvailability(lead.availability)){
    const [window='',detail='']=String(raw).split('|').map(x=>x.trim());const nw=norm(window),nd=norm(detail)
...
export default function InterestManagerV4(){
  const [leads,setLeads]=useState<Lead[]>([]),[slots,setSlots]=useState<Slot[]>([]),[loading,setLoading]=useState(true),[error,setError]=useState(''),[filter,setFilter]=useState('active'),[busy,setBusy]=useState('')
  ...
  const visible=enriched.filter(l=>filter==='all'?true:filter==='active'?['waiting','contacted','offered',''].includes(txt(l.status)):txt(l.status)===filter).sort((a,b)=>b.priority-a.priority)
  const perfect=enriched.filter(l=>l.best&&l.best.score>=95&&['waiting','contacted','offered',''].includes(txt(l.status))).length
  const followups=enriched.filter(l=>l.followup).length
  ...
  return <main className={styles.page}>
    ...
    <div className={styles.shell}>
      ...
      <section style={{display:'grid',gridTemplateColumns:'repeat(4,minmax(0,1fr))',gap:12,marginBottom:22}}>
        ...
        <div className={styles.smartStat}><span>Encaixes perfeitos</span><strong>{perfect}</strong><small>Compatibilidade alta com vagas</small></div>
        <div className={styles.smartStat}><span>Follow-ups pendentes</span><strong>{followups}</strong><small>Vagas oferecidas há mais de 24h</small></div>
      ...
      {!loading&&!error&&<section className={styles.smartLeadGrid}>
        {visible.map((lead,i)=>{const name=txt(lead.name)||'Interessado';const best=lead.best as Match|null;const follow=lead.followup;const msg=best?`Oi, ${name.split(' ')[0]}! Tudo bem? Aqui é o Marcos 😊 ${follow?'Passando para saber se você conseguiu ver minha mensagem anterior. ':''}Surgiu uma vaga para aula individual na ${best.slot.day.toLowerCase()}, às ${best.slot.time}, na modalidade ${best.slot.modality.toLowerCase()}. Você ainda tem interesse em iniciar?`:`Oi, ${name.split(' ')[0]}! Tudo bem? Aqui é o Marcos 😊 Você se cadastrou na minha lista de interesse para aulas individuais de canto.`;return <article className={styles.smartLeadCard} key={txt(lead.id)||`lead-${i}`}>
          <div className={styles.smartLeadTop}><div><div className={styles.smartNameRow}><strong>{name}</strong><span className={`${styles.statusPill} ${styles[`status_${txt(lead.status)||'waiting'}`]||''}`}>{statusLabel(lead.status)}</span><span style={{padding:'5px 8px',borderRadius:999,background:lead.priority>=90?'#332919':'#20262e',color:lead.priority>=90?'#f1cf88':'#aeb6c0',fontSize:9,fontWeight:900}}>{priorityLabel(lead.priority)}</span></div><p>{txt(lead.modality)||'Modalidade não informada'} • {txt(lead.startIntent)||'início não informado'}{lead.createdAt?` • há ${daysSince(lead.createdAt)} dias`:''}</p></div>{best?<div className={styles.matchBadge}><b>{best.score}%</b><span>{best.label}</span></div>:<div className={styles.noMatchBadge}>Sem encaixe agora</div>}</div>
