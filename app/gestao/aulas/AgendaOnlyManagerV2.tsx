...
export default function AgendaOnlyManagerV2(){
  ...
  async function load(){
    ...
    try{
      const [a,b]=await Promise.all([fetch('/api/admin/aulas',{cache:'no-store',signal:controller.signal}),fetch('/api/admin/alunos',{cache:'no-store',signal:controller.signal})])
      if(!a.ok||!b.ok)throw new Error('Não foi possível carregar a gestão.')
      const [ad,bd]=await Promise.all([a.json(),b.json()])
      setSlots(Array.isArray(ad.slots)?ad.slots:[]);setLeads(Array.isArray(ad.leads)?ad.leads:[]);setStudents(Array.isArray(bd.students)?bd.students:[])
  ...
  async function markOffered(lead:Lead){if(!lead.id)return;try{await post({action:'updateLeadStatus',id:lead.id,status:'offered'});setLeads(v=>v.map(l=>l.id===lead.id?{...l,status:'offered'}:l))}catch{}}
  async function fill(lead:Lead){if(!selected||!lead.id)return
    const modality=selected.modality==='Online ou Presencial'?(norm(lead.modality).includes('presencial')&&!norm(lead.modality).includes('online')?'Presencial':'Online'):selected.modality
    if(!confirm(`Preencher ${selected.day}, ${selected.time} (${modality}) com ${lead.name||'este interessado'}?\n\nO cadastro será convertido em aluno e essa vaga ficará ocupada.`))return
    setBusy(lead.id)
    try{await post({action:'fillSlotFromLead',leadId:lead.id,slotId:selected.id,modality});setSelected(null);await load()}catch(e:any){alert(e.message)}finally{setBusy('')}
  }

  const active=students.filter(s=>s.status==='active'),open=slots.filter(s=>s.status==='available'&&!s.studentId),waiting=leads.filter(l=>['waiting','contacted','offered',''].includes(text(l.status)))
  ...
  return <main className={styles.page}>
    ...
    {selected&&<div className={styles.backdrop} onClick={()=>setSelected(null)}><div className={styles.modal} onClick={e=>e.stopPropagation()}><div className={styles.modalHead}><div><span>HORÁRIO</span><h2>{selected.day} • {selected.time}</h2></div><button onClick={()=>setSelected(null)}>×</button></div><div style={{display:'flex',gap:8,flexWrap:'wrap',marginBottom:18}}><button onClick={()=>updateStatus('available')}>Disponível</button><button onClick={()=>updateStatus('occupied')}>Ocupado</button><button onClick={()=>updateStatus('blocked')}>Bloqueado</button><button onClick={remove}>Remover</button></div>{selected.status==='available'&&<div className={styles.matches}>{matches.map(({lead,score})=>{const name=text(lead.name)||'Interessado';return <article key={lead.id}><b>{score}%</b><div><strong>{name}</strong><span>{Array.isArray(lead.availability)?lead.availability.join(' • '):''}</span></div><div style={{display:'flex',gap:6,flexWrap:'wrap',justifyContent:'flex-end'}}>{lead.whatsapp&&<a onClick={()=>markOffered(lead)} href={wa(lead.whatsapp,`Oi, ${name.split(' ')[0]}! Tudo bem? Aqui é o Marcos Cruz 😊 Surgiu uma vaga ${selected.modality==='Online ou Presencial'?'online ou presencial':selected.modality.toLowerCase()} na ${selected.day.toLowerCase()}, às ${selected.time}. Você ainda tem interesse?`)} target="_blank">Oferecer vaga</a>}<button disabled={busy===lead.id} onClick={()=>fill(lead)} style={{padding:'10px 11px',borderRadius:10,border:0,background:'#9ce0bf',color:'#10251b',fontSize:10,fontWeight:900,cursor:'pointer'}}>{busy===lead.id?'Preenchendo...':'Preencher vaga'}</button></div></article>})}{!matches.length&&<div className={styles.bigEmpty}>Nenhum candidato compatível.</div>}</div>}</div></div>}
  </main>
}
